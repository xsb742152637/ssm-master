...
let MenuEx = ( function () {
    ...
    function select(mem, menu,type) {
        ...
        let menuChildren = Menu.getChildren(menu);
        for (let i = 0; i < Comm.getLength(menuChildren); i++) {
            let menuCh = Comm.getItem(menuChildren, i);
            Menu.removeMemChildrenAndSelf(menuCh, mem,type);
        }

        //处理menu节点所有上级勾选状态
        let menuParents = Menu.getParents(menu);
        //用反序遍历是因为Menu.getParents函数返回父祖级menu节点的集合是从上往下的，而这个地方需要从下往上
        for (let i = Comm.getLength(menuParents) - 1; i >= 0 ; i--) {
            //得到某个上级节点的子节点集合
            let sonMenuAuths = new Array();
            let m_p = Comm.getItem(menuParents, i);
            let sonMenus = Menu.getSingleLevelChildren(m_p);
            if(Comm.getLength(sonMenus) == 0 )
                continue;
            for (let j = 0; j < Comm.getLength(sonMenus); j++) {
            ...
            if (isAllAuth(sonMenuAuths)) {
                ...
                let c_mems = Mem.getChildren(mem);
                if(c_mems != null && Comm.getLength(c_mems) > 0){
                    let isHave = false;
                    for(let m = 0;m < Comm.getLength(c_mems);m++){
                        let c_mem = Comm.getItem(c_mems,m);
                        let c_sonMenuAuths = new Array();
                        for (let j = 0; j < Comm.getLength(sonMenus); j++) {
    ...
    function reverseSelect(mem, menu,type) {
        //处理上级菜单
        let menuParents = Menu.getParents(menu);
        for (let i = 0; i < Comm.getLength(menuParents); i++) {
            let p_item = Comm.getItem(menuParents, i);
        ...
        let memParents = Menu.findMemParents(menu, mem,type);
        for (let j = Comm.getLength(memParents) - 1; j >= 0; j--) {
            // 对mem节点进行剥离
            let p_item = Comm.getItem(memParents, j);
            let strippingMemItems = Mem.stripping(p_item, mem);
            //删除原mem节点
            Menu.removeMem(menu, p_item,type);
            //新增剥离后的mem
            for (let n = 0; n < Comm.getLength(strippingMemItems); n++) {
        ...
        //处理下级菜单
        let menuChildren = Menu.getChildren(menu);
        for (let i = Comm.getLength(menuChildren) - 1; i >= 0; i--) {
            let ch_item = Comm.getItem(menuChildren, i);
            //处理menu节点中含mem节点及其下级节点的情况,直接删除
    ...
    function menuStripping(menuOld,menu,mem,type) {
        ...
        let memParents = Menu.findMemParentsAndSelf(menu, mem,type);
        if(Comm.getLength(memParents) > 0){
            for (let j = Comm.getLength(memParents) - 1; j >= 0; j--) {
                let rp_item = Comm.getItem(memParents, j);
                //如果上级存在mem，删除mem及其下级mem，并为兄弟节点新增mem
                if(Comm.equals(rp_item,mem)){
                    Menu.removeMemChildrenAndSelf(menu, rp_item,type);
                }else{
                    //上级menu存在mem的上级，进行剥离
                    let strippingMemItems = Mem.stripping(rp_item, mem);
                    //删除原mem节点
                    Menu.removeMem(menu, rp_item,type);
                    //新增剥离剩下的mem
                    for (let i = 0; i < Comm.getLength(strippingMemItems); i++) {
            ...
            //得到该menu节点的兄弟节点
            let xd_items = Menu.getSingleLevelChildren(menu);
            for(let j = 0;j < Comm.getLength(xd_items);j++){
                let xd_item = Comm.getItem(xd_items,j);
                if(Comm.equals(xd_item,menuOld)){
                    //处理menu节点中含mem上级节点的情况,进行剥离
                    let memParents = Menu.findMemParents(xd_item, mem,type);
                    for (let jjj = Comm.getLength(memParents) - 1; jjj >= 0; jjj--) {
                        // 对mem节点进行剥离
                        let p_item = Comm.getItem(memParents, jjj);
                        let strippingMemItems = Mem.stripping(p_item, mem);
                        //删除原mem节点
            ...
            //得到该成员在该菜单中的所以下级
            let memChildrens = Menu.findMemChildren(menu, mem,type);
            if(Comm.getLength(memChildrens) > 0){
                for(let m = 0;m < Comm.getLength(memChildrens);m++){
                    //删除原mem节点
    ...
    function mergeMem(menus,mem,type){
        for(let o = Comm.getLength(menus)-1;o >=0;o--){
            let menuItem = Comm.getItem(menus,o);
            // console.log(menuItem);
            //对该menu节点的mem进行合并
            let mems = Menu.getAllMems(menuItem,type);
            // console.log("现有人员：");
            // console.log(mems);
            if(Comm.getLength(mems) < 1)
                continue;
            //得到该mem节点的所有上级
            let p_mems = Mem.getParents(mem);
            //从下往上循环
            for(let i = Comm.getLength(p_mems)-1;i >= 0;i--){
                mems = Menu.getAllMems(menuItem,type);
                let p_mem = Comm.getItem(p_mems,i);
                // console.log("上级节点：");
                // console.log(p_mem);
                let p_c_mem = Mem.getSingleLevelChildren(p_mem);

                // console.log("兄弟节点：");
                // console.log(p_c_mem);
                let j = 0;
                for(let m = 0;m < Comm.getLength(p_c_mem);m++){
                    let isHave = false;
                    for(let n = 0;n < Comm.getLength(mems);n++){
                    ...
                    if(!isHave){
                        let menus2 = Menu.getParents(menuItem);
                        for(let n = Comm.getLength(menus2)-1;n >=0;n--){
                            let menuItem2 = Comm.getItem(menus2,n);
                            let mems2 = Menu.getAllMems(menuItem2,type);
                            if(Comm.getLength(mems2) < 1)
                                continue;
                            for(let nn = Comm.getLength(mems2)-1;nn >=0;nn--){
    ...
    function  isAllAuth(sonMenus) {
        ...
        for (let i = 0; i < Comm.getLength(sonMenus); i++) {
    ...
    function setAuth(menu, mem,type) {
        ...
        if (Comm.getLength(Menu.findMemParentsAndSelf(menu, mem,type)) != 0) {
            ...
            //得到该菜单节点的所有子节点
            let sonMenus = Menu.getSingleLevelChildren(menu);
            ...
            if (Comm.getLength(sonMenus) == 0) {
                if (Comm.getLength(Menu.findMemChildren(menu, mem,type)) != 0) {
                    ...
                    let p_m = Menu.getParentsAndSelf(menu);
                    for(let i = 0;i < Comm.getLength(p_m);i++){
                ...
                let allAuth = isAllAuth(sonMenuAuths);
                ...
                if (allAuth == true) {
                    ...
                    let allNotAuth = isAllNotAuth(sonMenuAuths);
